...
unsigned char* USART_Receive_String(void)
{
	...
	for (i=0; i < Rec_Data_Length; i++)
	{
		...
		if (Rec_Buffer == '\0')
            break;		// If the uC receives nothing after 5 sec, break the loop
 ...
 * @Author: Mehdi
 *
 * @Params  timeOut: The selay time uC waits to receive data
...
uint8_t USART_DataAvailable(uint8_t timeOut)
{
	...
	dely_s(timeOut);    //Wait for timeOut sec to receive data through UART
...
{
/*	txWritePos = txWritePos + strlen(StringPtr);

	if (txWritePos >= TX_BUFFER_SIZE)
	{
        txWritePos = (txWritePos - TX_BUFFER_SIZE -1);
	}
